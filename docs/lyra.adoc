...
For value options, i.e. `--name=value`, one can specify a set of allowed
values. YOu can indicate the choices explicitly by specifying them to the
`choices()` call:
...
For either case the default, initial, value of the option is only modified
