...
int main()
{
	...
	auto parser = exe_name(config.process_name)
		| opt(config.file_name, "filename")["-o"]["--output"](
			"specifies output file")
		| opt(config.number, "an integral value")["-n"]
		| opt(
			[&](int i) {
				if (i < 0 || i > 10)
		...
		| opt(config.flag)["-f"]("A flag")
		| arg(config.first_pos, "first arg")("First position")
		| arg(config.second_pos, "second arg")("Second position");

	config = Config();
	{
		auto result = parser.parse({ "TestApp", "-o", "filename.ext" });
		test(REQUIRE(result))(REQUIRE(config.process_name == "TestApp"));
	}
	config = Config();
	{
		auto result = parser.parse({ "TestApp", "-o", "filename.ext" });
		test(REQUIRE(result))(REQUIRE(config.file_name == "filename.ext"));
	}
	config = Config();
	{
		auto result = parser.parse({ "TestApp", "-o=filename.ext" });
	...
	config = Config();
	{
		...
		test(REQUIRE(result))(REQUIRE(config.file_name == "%stdout"));
	}
	config = Config();
	{
		auto result = parser.parse({ "TestApp", "-n", "42" });
		test(REQUIRE(result))(REQUIRE(config.number == 42));
	}
	config = Config();
	{
		auto result = parser.parse({ "TestApp", "-n", "forty-two" });
		test(REQUIRE(!result))(REQUIRE(result.message()
			== "Unable to convert 'forty-two' to destination type"))(
			REQUIRE(config.number == 0));
	}
	config = Config();
	{
		auto result = parser.parse({ "TestApp", "-i", "3" });
		test(REQUIRE(result))(REQUIRE(config.index == 3));
	}
	config = Config();
	{
	...
	config = Config();
	{
		...
		test(REQUIRE(result))(REQUIRE(config.flag));
	}
	config = Config();
	{
		auto result = parser.parse({ "TestApp" });
		test(REQUIRE(result))(REQUIRE(result.value().type()
			== parser_result_type::matched))(REQUIRE(config.flag == false));
	}
	config = Config();
	{
	...
	config = Config();
	{
		...
		test(REQUIRE(result))(REQUIRE(config.flag == false))(
			REQUIRE(config.first_pos == "something"));
	}
	config = Config();
	{
		auto result
			= parser.parse({ "TestApp", "-f", "1st", "-o", "filename", "2nd" });
		test(REQUIRE(result))(REQUIRE(config.first_pos == "1st"))(
