...
template <class... A>
scope & test_main(
	scope & s, char const * file, int line, bool success, A... args)
{
	...
	condition += "0})";
	s(test_main_f(main_argc, main_args) == 0 ? success : !success,
		condition.c_str(), file, line);
	return s;
}
}} // namespace bfg::mini_test
#	define main test_main_f
#	define TEST_MAIN(S, ...) \
		test_main(S, __FILE__, __LINE__, true, __VA_ARGS__)
#	define TEST_MAIN_FAIL(S, ...) \
		test_main(S, __FILE__, __LINE__, false, __VA_ARGS__)
