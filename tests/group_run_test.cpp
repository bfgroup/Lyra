...
int main(int, const char **)
{
	...
	{
		auto cli = lyra::cli() | group().add_argument(literal("one"))
			| group().add_argument(literal("two"));
		test(REQUIRE(cli.parse({ "TestApp", "one" })))(REQUIRE(cli.parse(
			{ "TestApp", "two" })))(REQUIRE(cli.parse({ "TestApp" })));
	}
	{
		std::string one = "one";
		int two = 2;
		auto cli = lyra::cli()
			| group()
				  ...
				  .add_argument(arg(one, "one"))
			| group()
				  .sequential()
				  .add_argument(literal("two"))
				  .add_argument(arg(two, "two"));
